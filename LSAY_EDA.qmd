...
LEGIT_TEACHERS_2006 <- LSAY2006 %>%
  ...
  # Just checking that the variables are meant to go from 011 to 024, to 029
...
# NP: It could be worth adding in the counts for each field here
aos_2006 <- data.frame(
...
base_chart <- ggplot(data_2003, aes(x = as.factor(Percentiles), y = Score, fill = as.factor(teacher))) +
  ...
  labs(title = "Teachers at 15 years-old were slightly more likely to be proficient in maths compared to the general population",
       subtitle = glue("PISA 2003 Mathematical Proficiency scores for {colour_text(grattan_red, 'teachers')} and {colour_text(grattan_orange, 'non-teachers')} across percentiles"),
       x = "Percentiles",
       fill = "Teacher") +
   geom_hline(yintercept = 482, linetype = "dotted", color = "black", size = 1) +
  annotate(geom = "text", y = 484, x = "90", vjust = "bottom", hjust = -.6, label = "Proficiency\nbenchmark")
...
base_chart +
  ...
  scale_x_discrete(expand = expansion(mult = c(0, 0.6)))
...
#NP: Normally, I would soft code in the values by pulling them from the flex table produced at the start of this document. For the purposes of this analysis, I think hard coding is okay. 
