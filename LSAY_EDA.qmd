...
```{r}
library(survey)
# attempting to subset the data to just those working/worked as PS teacher & calculate summary statistics
teacher_ids <- UNIQUE_LEGIT_TEACHERS$STIDSTD
teacher_data <- LSAY2003[LSAY2003$STIDSTD %in% teacher_ids, ]
teacher_design <- svydesign(ids = ~1, weights = ~W_FSTUWT, data = teacher_data)

# calculating for each plausible value
...
# need to fact check that this accounts for the weights properly after subsetting the data to just those working as PS teachers
