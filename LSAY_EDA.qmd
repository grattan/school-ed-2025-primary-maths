...
# Creating table to display results
 unique_teachers_yo3_table <- kbl(unique_teachers_yo3, caption = "Number of Teachers in the Y03 Data.") %>%
  kable_paper() %>%
  row_spec(0, background = "orange", bold = TRUE) %>%
  scroll_box(width = "1200px", height = "400px")
...
teacher_jobs <- LSAY2003[LSAY2003$STIDSTD %in% unique_teachers_yo3$STIDSTD, ]
...
# Combine datasets
data_combined <- bind_rows(data_2003, data_2006, data_2009, data_2015)
...
data_combined |> 
  ggplot(aes(y= teacher, xdist = dist_normal(Score,))) +
  stat_interval() +
  scale_color_brewer()
...
```{r}
# getting the sample of 'true' teachers extracted.
sample_sizes_2003 <- LSAY2003_teachflag %>%
  group_by(teacher) %>%
  summarize(n = n())

sample_sizes_2006 <- LSAY2006_teachflag %>%
  group_by(teacher) %>%
  summarize(n = n())

sample_sizes_2009 <- LSAY2009_teachflag %>%
  group_by(teacher) %>%
  summarize(n = n())

sample_sizes_2015 <- LSAY2015_teachflag %>%
  group_by(teacher) %>%
  summarize(n = n())

# soft code the percentages
percentages <- c(84.05, 85.01, 86.99, 73.89)
sample_sizes <- c(sample_sizes_2003$n, sample_sizes_2006$n, sample_sizes_2009$n, sample_sizes_2015$n)

proportions <- percentages / 100

abovprof <- proportions * sample_sizes

total_abovprof <- sum(abovprof)
total_sample_size <- sum(sample_sizes)

pooled_proportion <- total_abovprof / total_sample_size

pooled_percentage <- pooled_proportion * 100
