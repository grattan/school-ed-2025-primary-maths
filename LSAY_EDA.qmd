...
Starting with 2003 first as it has the largest sample of teachers. 
looking at other professions at the end of the waves.
...
UNIQUE_PROFESSIONALS_Y03 <- LSAY2003 |>
  ...
  # Mutate across the observation to get the different professions
  mutate(across(c(LKD029), ~ case_when(
    . == '2412' ~ 1,  # Teacher = 1
    startsWith(., '253')  ~ 2,  # Medical Practitioners = 2
    startsWith(., '254')  ~ 3,  # Midwifery and Nursing Professionals = 3
    startsWith(., '271')  ~ 4,  # Legal Professionals = 4
    startsWith(., '221')  ~ 5,  # Accounts Auditors and Company Secretaries = 5
    startsWith(., '233')  ~ 6,  # Engineering Professionals = 6
    is.na(.) ~ NA_integer_, # NA values retained
    TRUE ~ 0 # All others = 0
  ))) %>%
  # New column to check if the observation is one of the targeted professions
  mutate(legit_professions = if_any(c(LKD029), ~ . %in% 1:6)) %>%
  filter(legit_professions) # Filtering for just the IDs with professions
...
profession_frequency <- UNIQUE_PROFESSIONALS_Y03 %>%
  ...
  mutate(
    Profession = case_when(
      LKD029 == 1 ~ 'Teacher',
      LKD029 == 2 ~ 'Medical Practitioners',
      LKD029 == 3 ~ 'Midwifery and Nursing Professionals',
      LKD029 == 4 ~ 'Legal Professionals',
      LKD029 == 5 ~ 'Accounts Auditors and Company Secretaries',
      LKD029 == 6 ~ 'Engineering Professionals'
    )
  )
```
