...
# How many school leavers enroll in primary school teaching?

To understand how many people who studied primary school teaching, we would select the IDs of those who were studying to be teachers then also select variables associated with question: "Left school before completing Year 12".

```{r}
# IDs of those enrolled in primary school aos at some point
studying_teachers <- unique(unique_studying_teachers_y03$STIDSTD)
# Filter for those studying primary school teaching and leaving school variables.
...
The student IDs of these two: 2999 and 10049.
...
teachers_2006 <- LSAY2006 %>%
  ...
  mutate(teachers_2006 = if_any(c(LBD011, LCD024, LDD024, LED029, LFD029, LGD029, LHD029, LID029, LJD029, LKD029), ~ . == 1)) %>%
...
```{r}
# extracting the IDs of those who were teachers
studying_teachers_06 <- unique(teachers_2006$STIDSTD)
...
professionals_vis <- professionals %>%
  ...
  arrange(Mean_Math) %>%
  mutate(Profession = factor(Profession, levels = unique(Profession))) # Reorder Profession factor

profession_visualisation <- professionals_vis %>%
  ggplot(aes(y = Profession, xdist = dist_normal(Mean_Math, Std_Math))) +
  stat_interval() +
  geom_point(aes(x = Mean_Math), size = 3, color = "black") +
  labs(
    title = "Teachers at 15 years-old were slightly more likely to be proficient in math compared to the general population but less than other professionals",
    subtitle = glue("Distribution of {colour_text('#FF0000','Pisa 2003')} Mathematical Proficiency scores for teachers and other professionals"),
    x = "Score",
    y = "Teacher",
    caption = "Source: Longitudinal Surveys of Australian Youth, 2003 cohort (Version 7.1)"
  ) +
  geom_vline(xintercept = 482, linetype = "dotted", color = "red", size = 1) + 
  annotate(geom = "text", y = 1.5, x = 475, colour = "black", vjust = 0.5, hjust = -.15, label = "Proficiency benchmark", size =2) + 
  xlim(300, 750)

profession_visualisation +
  theme_grattan(chart_type = "bar", flipped = TRUE) +
...
distributions_2003 <- visualisation_2003 |> 
  ...
  labs(title = "Teachers at 15 years-old were slightly more likely to be proficient in math compared to the general population",
       subtitle = glue("Distribution of {colour_text('#FF0000','Pisa 2003')} Mathematical Proficiency scores for teachers and non-teachers"),
       x = "Score",
       y = "Teacher",
  caption = "Source: Longitudinal Surveys of Australian Youth, 2003 cohort (Version 7.1)") +
...
Another chart showing distributions for teacher non teacher across proficiency levels \* potential chart for other professionals (nice to have)

```{r}
# creating flags for each professional to get proficiency scores
#2003
LSAY2003_profession_flag <- LSAY2003 %>%
  mutate(LKD029 = as.character(LKD029)) %>%
  mutate(profession = case_when(
...
# this seems to be too computationally heavy because of the 6 professional groups
professionals_2003 <- Rrepest::Rrepest(
  data = LSAY2003_profession_flag,
  svy = "PISA",
  est = est(c("mean","std"),"pv@math"),
  by = c("profession"),
...
LSAY2003_profession_flag <- LSAY2003 %>%
  ...
  mutate(profession = case_when(
    LKD029 == '2412' ~ 'Teacher',
    startsWith(LKD029, '253') ~ 'Medical Practitioners',
    TRUE ~ 'Other'  # Limit to just a couple of groups for testing
  ))

# getting the sample sizes for each profession group
sample_sizes2003 <- LSAY2003_profession_flag %>% 
  count(profession, name = "sample_size")

# joining the sample size to the professionals_2003
professionals_2003 <- professionals_2003 %>%
  left_join(sample_sizes2003, by = "profession")

# adding a year column as the second variable
professionals_2003 <- professionals_2003 %>%
  mutate(Year = 2003) %>%
  select(Year, everything())
...
# 2006
LSAY2006_profession_flag <- LSAY2006 %>%
  mutate(LKD029 = as.character(LKD029)) %>%
  mutate(profession = case_when(
    LKD029 == '2412' ~ 'Teacher',
    startsWith(LKD029, '253') ~ 'Medical Practitioners',
...
professionals_2006 <- Rrepest::Rrepest(
  data = LSAY2006_profession_flag,
  svy = "PISA",
  est = est(c("mean","std"),"pv@math"),
  by = c("profession"),
  cm.weights = c(paste0("w_fstr", 1:80)),
...
# getting the sample sizes for each profession group
sample_sizes2006 <- LSAY2006_profession_flag %>% 
  count(profession, name = "sample_size")

# joining the sample size to the professionals_2003
professionals_2006 <- professionals_2006 %>%
  left_join(sample_sizes2003, by = "profession")
...
```{r}
#2009
LSAY2009_profession_flag <- LSAY2009 %>%
  mutate(LKD029 = as.character(LKD029)) %>%
  mutate(profession = case_when(
    LKD029 == '2412' ~ 'Teacher',
    startsWith(LKD029, '253') ~ 'Medical Practitioners',
...
professionals_2009 <- Rrepest::Rrepest(
  data = LSAY2009_profession_flag,
  svy = "PISA",
  est = est(c("mean","std"),"pv@math"),
  by = c("profession"),
  cm.weights = c(paste0("w_fstr", 1:80)),
...
# getting the sample sizes for each profession group
sample_sizes2009 <- LSAY2009_profession_flag %>% 
  count(profession, name = "sample_size")

# joining the sample size to the professionals_2003
professionals_2009 <- professionals_2009 %>%
  left_join(sample_sizes2009, by = "profession")
...
LSAY2015_profession_flag <- LSAY2015 %>%
  mutate(LHD022 = as.character(LHD022)) %>%
  mutate(profession = case_when(
    LHD022 == '2412' ~ 'Teacher',
    startsWith(LHD022, '253') ~ 'Medical Practitioners',
    startsWith(LHD022, '254') ~ 'Midwifery and Nursing Professionals',
    startsWith(LHD022, '271') ~ 'Legal Professionals',
    startsWith(LHD022, '221') ~ 'Accounts Auditors and Company Secretaries',
    startsWith(LHD022, '233') ~ 'Engineering Professionals',
    TRUE ~ 'Other'
  )) 

professionals_2015 <- Rrepest::Rrepest(
  data = LSAY2015_profession_flag,
  svy = "PISA",
  est = est(c("mean","std"),"pv@math"),
  by = c("profession"),
  cm.weights = c(paste0("w_fsturwt",1:80)),
...
# getting the sample sizes for each profession group
sample_sizes2015 <- LSAY2015_profession_flag %>% 
  count(profession, name = "sample_size")

# joining the sample size to the professionals_2003
professionals_2015 <- professionals_2015 %>%
  left_join(sample_sizes2015, by = "profession")
...
professionals_all <- bind_rows(professionals_2003, professionals_2006, professionals_2009, professionals_2015)

# creating weighted average proficiency for each year
# getting total sample size
total_sample <- sum(LSAY_agprof_nt$sample_size)

professionals_aggprof <- professionals_all %>%
  mutate(weighted_prof = (`b.mean.pv@math` * sample_size) / total_sample)
...
```{r}
# adding years to the df
professionals2003_agprof <- professionals_2003 %>%
  mutate(Year = 2003)
professionals2006_agprof <- professionals_2006 %>%
  mutate(Year = 2006)
professionals2009_agprof <- professionals_2009 %>%
  mutate(Year = 2009)
professionals2015_agprof <- professionals_2015 %>%
  mutate(Year = 2015)

#joining all these dataframes together
professionals_agprof <- bind_rows(professionals2003_agprof, professionals2006_agprof, professionals2009_agprof, professionals2015_agprof)

#dropping last two columns
LSAY_agprof <- professionals_agprof %>%
  select(-c("b.std.pv@math", "se.std.pv@math"))
# rearranging so year column is at the start
LSAY_agprof <- LSAY_agprof %>%
  select(Year, everything())

# getting all the n_accounts/auditors/company secretaries
n_accounts_2003 <- sum(LSAY2003_profession_flag$profession == 'Accounts Auditors and Company Secretaries')
n_accounts_2006 <- sum(LSAY2006_profession_flag$profession == 'Accounts Auditors and Company Secretaries')
n_accounts_2009 <- sum(LSAY2009_profession_flag$profession == 'Accounts Auditors and Company Secretaries')
n_accounts_2015 <- sum(LSAY2015_profession_flag$profession == 'Accounts Auditors and Company Secretaries')

# engineering professionals
n_eng_2003 <- sum(LSAY2003_profession_flag$profession == 'Engineering Professionals')
n_eng_2006 <- sum(LSAY2006_profession_flag$profession == 'Engineering Professionals')
n_eng_2009 <- sum(LSAY2009_profession_flag$profession == 'Engineering Professionals')
n_eng_2015 <- sum(LSAY2015_profession_flag$profession == 'Engineering Professionals')

# legal professionals
n_legal_2003 <- sum(LSAY2003_profession_flag$profession == 'Legal Professionals')
n_legal_2006 <- sum(LSAY2006_profession_flag$profession == 'Legal Professionals')
n_legal_2009 <- sum(LSAY2009_profession_flag$profession == 'Legal Professionals')
n_legal_2015 <- sum(LSAY2015_profession_flag$profession == 'Legal Professionals')

# medical practitioners
n_medical_2003 <- sum(LSAY2003_profession_flag$profession == 'Medical Practitioners')
n_medical_2006 <- sum(LSAY2006_profession_flag$profession == 'Medical Practitioners')
n_medical_2009 <- sum(LSAY2009_profession_flag$profession == 'Medical Practitioners')
n_medical_2015 <- sum(LSAY2015_profession_flag$profession == 'Medical Practitioners')

# Midwifery and Nursing Professionals
n_midwifery_2003 <- sum(LSAY2003_profession_flag$profession == 'Midwifery and Nursing Professionals')
n_midwifery_2006 <- sum(LSAY2006_profession_flag$profession == 'Midwifery and Nursing Professionals')
n_midwifery_2009 <- sum(LSAY2009_profession_flag$profession == 'Midwifery and Nursing Professionals')
n_midwifery_2015 <- sum(LSAY2015_profession_flag$profession == 'Midwifery and Nursing Professionals')

# Other
n_other_2003 <- sum(LSAY2003_profession_flag$profession == 'Other')
n_other_2006 <- sum(LSAY2006_profession_flag$profession == 'Other')
n_other_2009 <- sum(LSAY2009_profession_flag$profession == 'Other')
n_other_2015 <- sum(LSAY2015_profession_flag$profession == 'Other')

# finding weighted average of someone in the accounts auditors & companies sec profession
LSAY_agprof_ac <- LSAY_agprof %>%
  filter(profession == 'Accounts Auditors and Company Secretaries') %>%
  #adding a column for n = teachers 'n_teachers_2003' for each year
  mutate(n_accounts = case_when(
    Year == 2003 ~ n_accounts_2003,
    Year == 2006 ~ n_accounts_2006,
    Year == 2009 ~ n_accounts_2009,
    Year == 2015 ~ n_accounts_2015
  )) 

# creating a weighted average proficiency for each year
# getting sum of all teacher samples
total_accounts <- sum(LSAY_agprof_ac$n_accounts)

LSAY_agprof_ac <- LSAY_agprof_ac %>%
  mutate(weighted_prof = (`b.mean.pv@math` * n_accounts) / total_teachers)
...
boxplot_stats <- function(data, year) {
  # filter data for teachers
  filtered_data <- data[data$teacher == TRUE, ]
  
  boxplot_data <- pisa.per.pv(
    pvlabel = paste0("PV", 1:5, "MATH"), 
    per = c(25, 50, 75), 
    data = LSAY2003_teachflag,
    by = teacher
  )
...
# compute boxplots for each year
boxplot_2003 <- compute_boxplot_stats(LSAY2003_teachflag, 2003)
boxplot_2006 <- compute_boxplot_stats(LSAY2006_teachflag, 2006)
boxplot_2009 <- compute_boxplot_stats(LSAY2009_teachflag, 2009)
boxplot_2015 <- compute_boxplot_stats(LSAY2015_teachflag, 2015)

# combine the dfs
pisa_boxplot_stats <- rbind(boxplot_2003, boxplot_2006, boxplot_2009, boxplot_2015)
...
```{r}
teacher_fact <- ggplot(pisa_boxplot_stats, aes(x = factor(Year), ymin = Lower_whisker, lower = Q1, middle = Q2, upper = Q3, ymax = Upper_whisker)) +
  geom_boxplot(stat = "identity") +
  facet_wrap(~ teacher) +
  labs(x = "Year", y = "Scores", title = "Boxplot of Scores Faceted by Teacher Status") +
  theme_minimal() +
  coord_flip()
...
```{r}
non_teacher_boxplot <- ggplot(pisa_boxplot_stats, aes(x = as.factor(Year))) +
  geom_boxplot(aes(ymin = Lower_whisker,lower = Q1, middle = Q2, upper = Q3, ymax = Upper_whisker), stat = "identity") +
  theme_minimal() +
  labs(title = "Boxplot of Math Scores by Year", x = "Year", y = "Score") + coord_flip()
