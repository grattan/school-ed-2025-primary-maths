...
LSAY2003_teachflag <- 
  LSAY2003 |> 
  mutate(teacher = if_else(STIDSTD %in% teacher_ids, T, F))
...
# Create the table
teachers_below_profstand_y03 <- table(teacher_means_03$below_prof_avg)
names(teachers_below_profstand_y03) <- c("Above Proficiency", "Below Proficiency")


teachers_below_profstand_y03_df <- as.data.frame(teachers_below_profstand_y03) 


proficiency_y03_table <- kable(teachers_below_profstand_y03, caption = "Proficiency Status of Teachers in 2003", col.names = c("Proficiency Status", "Count"), align = "c", booktabs = TRUE) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>%
  row_spec(0, background = "orange", color = "black") %>%
  column_spec(column = c(1,2), color = "black")

proficiency_y03_table
```
...
```{r}
studying_teachers_06 <- unique(STUDYING_TEACHERS_2006$STIDSTD)
# filter the LSAY2003 data for those IDs and high school leaving variables.
...
# mean math scores for each teacher across each pv first
mean_pv1_by_teacher_06 <- svyby(~PV1MATH, ~STIDSTD, teacher_design_y06, svymean)
mean_pv2_by_teacher_06 <- svyby(~PV2MATH, ~STIDSTD, teacher_design_y06, svymean)
mean_pv3_by_teacher_06 <- svyby(~PV3MATH, ~STIDSTD, teacher_design_y06, svymean)
mean_pv4_by_teacher_06 <- svyby(~PV4MATH, ~STIDSTD, teacher_design_y06, svymean)
mean_pv5_by_teacher_06 <- svyby(~PV5MATH, ~STIDSTD, teacher_design_y06, svymean)

# creating a data frame with ID and mean for each PV for each ID
teacher_means_y06 <- data.frame(
  STIDSTD = mean_pv1_by_teacher_06$STIDSTD,
  mean_pv1 = mean_pv1_by_teacher_06$PV1MATH,
  mean_pv2 = mean_pv2_by_teacher_06$PV2MATH,
  mean_pv3 = mean_pv3_by_teacher_06$PV3MATH,
  mean_pv4 = mean_pv4_by_teacher_06$PV4MATH,
  mean_pv5 = mean_pv5_by_teacher_06$PV5MATH
)
...
# mean math scores for each teacher across each pv first
mean_pv1_by_teacher_y09 <- svyby(~PV1MATH, ~STIDSTD, teacher_design_y09, svymean)
mean_pv2_by_teacher_y09 <- svyby(~PV2MATH, ~STIDSTD, teacher_design_y09, svymean)
mean_pv3_by_teacher_y09 <- svyby(~PV3MATH, ~STIDSTD, teacher_design_y09, svymean)
mean_pv4_by_teacher_y09 <- svyby(~PV4MATH, ~STIDSTD, teacher_design_y09, svymean)
mean_pv5_by_teacher_y09 <- svyby(~PV5MATH, ~STIDSTD, teacher_design_y09, svymean)

# creating a data frame with ID and mean for each PV for each ID
teacher_means_y09 <- data.frame(
  STIDSTD = mean_pv1_by_teacher_y09$STIDSTD,
  mean_pv1 = mean_pv1_by_teacher_y09$PV1MATH,
  mean_pv2 = mean_pv2_by_teacher_y09$PV2MATH,
  mean_pv3 = mean_pv3_by_teacher_y09$PV3MATH,
  mean_pv4 = mean_pv4_by_teacher_y09$PV4MATH,
  mean_pv5 = mean_pv5_by_teacher_y09$PV5MATH
)
...
There seems to be a large number of teachers still above the proficient standard in 2009. However the mean score was [generally high for Australia](https://www.acer.org/files/PISA-2009-In-Brief.pdf)
...
# Key findings across the years

(at the moment focus on Y03, Y06 and Y09)
...
Seeing how this compare against everyone more generally (how each state performed)

```{r echo=FALSE}
#LSAY2003
library(intsvy)
pisa_state_03 <- pisa.mean.pv(pvlabel = paste0("PV", 1:5, "MATH"), by = "STATEID", data = LSAY2003) %>%
  mutate(STATE = c("ACT", "NSW", "VIC", "QLD", "SA", "WA", "TAS", "NT"))

 pisa_state_2003 <- kbl(pisa_state_03, caption = "Average Math Performance Across States, 2003") %>%
  kable_paper() %>%
  row_spec(0, background = "orange", bold = TRUE) %>%
  scroll_box(width = "1000px", height = "300px")
pisa_state_2003
...
#LSAY2006
library(intsvy)
pisa_state_06 <- pisa.mean.pv(pvlabel = paste0("PV", 1:5, "MATH"), by = "STATE", data = LSAY2006) %>%
  mutate(STATE = c("NSW", "VIC", "QLD", "SA", "WA", "TAS", "NT", "ACT"))

 pisa_state_2006 <- kbl(pisa_state_06, caption = "Average Math Performance Across States, 2006") %>%
  kable_paper() %>%
  row_spec(0, background = "orange", bold = TRUE) %>%
  scroll_box(width = "1000px", height = "300px")
pisa_state_2006
...
```{r echo=FALSE}
#LSAY2009
pisa_state_09 <- pisa.mean.pv(pvlabel = paste0("PV", 1:5, "MATH"), by = "STATE", data = LSAY2009) %>%
  mutate(STATE = c("NSW", "VIC", "QLD", "SA", "WA", "TAS", "NT", "ACT"))

 pisa_state_2009 <- kbl(pisa_state_09, caption = "Average Math Performance Across States, 2009") %>%
  kable_paper() %>%
  row_spec(0, background = "orange", bold = TRUE) %>%
  scroll_box(width = "1000px", height = "300px")
pisa_state_2009
...
# Exploring State Averages Across the years.

Looking at how the states performed on average in maths over the years using `intsvy` package.

```{r echo= FALSE}
library(dplyr)
library(stringr)

# define the mathcut values
mathcut <- c(358, 420, 482, 545, 607, 669)

# calculate the mean performance by state (intsvy package)
pisa_state_03 <- pisa.ben.pv(pvlabel = paste0("PV", 1:5, "MATH"), cutoff = mathcut, by = "STATEID", data = LSAY2003)

# eeplace STATEID with actual state acronyms using case_when
pisa_state_03 <- pisa_state_03 %>%
  mutate(STATEID = case_when(
    STATEID == 1 ~ "NSW",
    STATEID == 2 ~ "VIC",
    STATEID == 3 ~ "QLD",
    STATEID == 4 ~ "SA",
...
# checking format of string for benchmarks
unique_benchmarks <- unique(pisa_state_03$Benchmark)


# Use the exact string observed in the unique values
above_482 <- pisa_state_03 %>%
  filter(Benchmark == "At or above 482") %>%
  select(STATEID, Percentage)

 above_482_table <- kbl(above_482, caption = "Percentage Above 482 - States, 2003") %>%
  kable_paper() %>%
  row_spec(0, background = "orange", bold = TRUE) 
above_482_table
```
...
```{r echo= FALSE}
# calculate the mean performance by state (intsvy package)
pisa_state_06 <- pisa.ben.pv(pvlabel = paste0("PV", 1:5, "MATH"), cutoff = mathcut, by = "STATE", data = LSAY2006)

# eeplace STATEID with actual state acronyms using case_when
pisa_state_06 <- pisa_state_06 %>%
...
# checking format of string for benchmarks
unique_benchmarks <- unique(pisa_state_06$Benchmark)


# Use the exact string observed in the unique values
above_482_2006 <- pisa_state_06 %>%
  filter(Benchmark == "At or above 482") %>%
  select(STATE, Percentage)

 above_482_2006_table <- kbl(above_482_2006, caption = "Percentage Above 482 - States, 2006") %>%
  kable_paper() %>%
  row_spec(0, background = "orange", bold = TRUE) 
above_482_2006_table
```

```{r echo=FALSE}
# calculate the mean performance by state (intsvy package)
pisa_state_09 <- pisa.ben.pv(pvlabel = paste0("PV", 1:5, "MATH"), cutoff = mathcut, by = "STATE", data = LSAY2009)

# replace STATEID with actual state acronyms using case_when
pisa_state_09 <- pisa_state_09 %>%
  mutate(STATE = case_when(
    STATE == 1 ~ "NSW",
    STATE == 2 ~ "VIC",
    STATE == 3 ~ "QLD",
...
# checking format of string for benchmarks
unique_benchmarks <- unique(pisa_state_09$Benchmark)


# Use the exact string observed in the unique values
above_482_2009 <- pisa_state_09 %>%
  filter(Benchmark == "At or above 482") %>%
  select(STATE, Percentage)

 above_482_2009_table <- kbl(above_482_2009, caption = "Percentage Above 482 - States, 2009") %>%
  kable_paper() %>%
  row_spec(0, background = "orange", bold = TRUE) 
above_482_2009_table
```
