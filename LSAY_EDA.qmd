...
Nice to have: Compare this to all non-teachers and other professionals.
...
Proficient / non-proficient chart for teachers, non teachers and professionals - bar chart
- ONLY FOR PROFICIENT/MEETING PROFICIENCY
...
Another chart showing distributions for teacher non teacher across proficiency levels
* potential chart for other professionals (nice to have)
...
professionals_2006 <- left_join(sample_sizes2006, by = "profession") %>%
  mutate(Year = 2006) %>%
  select(Year, everything())
...
```{r}
pisa.per.pv(pvlabel= paste0("PV",1:5,"MATH"),
per=c(10, 25, 75, 90), data= LSAY2003)
```
