...
```{r}
teacher_fact <- ggplot(pisa_boxplot_stats, 
                       aes(x = teacher, 
                           ymin = Lower_whisker, lower = Q1, 
                           middle = Q2, upper = Q3, ymax = Upper_whisker)) +
  geom_boxplot(stat = "identity") +
  facet_grid(Year ~ .) +  
  labs(x = "Teacher Status", y = "Scores", title = "Teachers tend to perform better than non-teachers across the years", subtitle = "Distribution of Australian Year 10 PISA scores in Mathematical Literacy") +
  scale_x_discrete(labels = c("TRUE" = "Teacher", "FALSE" = "Non-Teacher")) +  
  theme_minimal() +
  coord_flip()
