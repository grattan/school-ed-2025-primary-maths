...
```{r}
file_path_course_information <- "data/UAC courses reference data - ua15 to ua24.xlsx"

data_frame_course_information <- file_path_course_information |> 
  excel_sheets() |> 
  # Filter out 2024 and 2015 just for simplicity of common structure across sheets
  # Use all years in final analysis
  setdiff(c("2024", "2015")) |> 
  set_names() |> 
  map(read_excel, path = file_path_course_information) |> 
  list_rbind()

file_path_school_leaver_results <- "data/UAC data for Grattan Institute - 2015 to 2024.xlsx"
