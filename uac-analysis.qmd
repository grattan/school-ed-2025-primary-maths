...
#May need to do a tibble per subject. TBC if we care about every subject
```{r}
...
```{r}
# Create the data to make a stacked bar chart of the proportion of students in each band for each subject, by type of teaching degree enrolment. Define this more narrowly given that adding the eithers and boths is marginal to the analysis (set_groups2). 
...
# Calculate the proportion of students in each band for the following subject variables (std1_band, st2_band, adv_band, ext1_num_band, ext2_num_band), by type of teaching degree enrolment (set_groups2). Output a tibble with columns for group_name (the values of set_groups2), subject (with the selected variables as the values), band (the levels of the selected variables) and band_proportion (proportion of students in that band, for that variable and group_name).

### Too hard basket for co-pilot, probably need to step it out more/differently

# Calculate the proportion of students in each band for each subject (variable ending in "band"), by type of teaching degree enrolment (set_groups). Use set_groups conditions as defined above.

# Function to calculate proportions for each 'band' variable individually
calculate_band_proportion <- function(group) {
  band_proportion <- classified_school_leaver_results |> 
    filter(enrolment_status == "E") |> 
    filter(
      if (group == "") {
        TRUE  # Include all groups
    ...
    select(ends_with("band")) |> 
    map(~ {
      # Check if the variable has any data
      tbl <- table(.x)
      if (length(tbl) <= 1) {
        return(NA)  # Return NA if there's insufficient data for proportion calculations
      } else {
        return(round(prop.table(tbl) * 100, 0))  # Calculate the proportion
      }
    })
  
  return(band_proportion)
}
...
# Print the data in each of the tables in the band_proportion_groups list

#View(band_proportion_groups$band_proportion)
