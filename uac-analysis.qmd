  ...
  participation <- classified_school_leaver_results |> 
    ...
    filter(
      if (set_group == "") {
        ...
        teaching_degree %in% str_split(set_group, "\\|")[[1]]  # According to ChatGPT, necessary to give the if / else function the instruction to split strings by interpreting '|' as "or". It doesn't work using c(x, y) -  runs multiple string searches, not one with multiple conditions.
      }
    ) |> 
...
# Create a tibble with mean_score data for each set_group value
...
ggplot(mean_long, aes(x = subject, y = percentage, fill = set_group_name)) +
  geom_col(position = "dodge") +  # Use dodge to create grouped columns
...
#Create new variables in classified_school_leaver_results. First, create a variable mirroring ext1_band, as a factor, but with the letter 'E' removed from the front of all values that include it, and with the levels redefined as 1, 2, 3 and 4. Then do the same for the variable ext2_band.
