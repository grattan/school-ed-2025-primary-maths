...
To weight responses, we use the same method as that employed by [@socialresearchcentre2023].
...
We cannot do this first step, as we lack numerical data about the selection mechanisms which would or would not cause people to respond to our survey. [@socialresearchcentre2023] similarly could not calculate base weights in their survey of educators.
...
data.frame(x = weights(survey_data_raked)) |> 
  ...
  labs(title = "The distribution of weights is left-skewed",
       subtitle = "Distribution of person-unit weights",
       x = "Person-level weight\n(approximate number of teachers in population represented per respondent)", y = NULL,
