...
library(RALSA) # install.packages("RALSA", dependencies = TRUE)
# reading sav file
...
# Alternative #1: EdSurvey package
# Attempts to try download the PISA data using EdSurvey failed as the website structure has changed since the package was released. I downloaded manually instead.
# options(HTTPUserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/42.0.2311.135 Safari/537.36 Edge/12.246")
# downloadPISA(root = "data", years = c(2003))
# pisa2003_EdSurvey <- readPISA(path = "data/PISA/2003", 
#                      database = "INT", 
#                      countries = "AUS", 
#                      cognitive = "score")

# Alternative #2: Using RALSA
lsa.convert.data(inp.folder = "data/PISA/2003",
                 ISO = c("aus"),
                 out.folder = "data/PISA/2003")
```
...
PISA2003_uppercase <- PISA2003

names(PISA2003_uppercase) <- toupper(names(PISA2003))

# Values slightly different from Ainley
means_2003_new <- pisa.mean.pv(
  pvlabel = paste0("PV", 1:5, "MATH"),
  by = "ST01Q01",
  data = PISA2003_uppercase
)

# Manually calculate means without repetition
PISA2003_uppercase <-
  PISA2003_uppercase |>
...
PISA2003_manual_means <- 
  PISA2003_uppercase |> 
  group_by(grade_category) |> 
  summarise(across(c("PV1MATH", "PV2MATH", "PV3MATH", "PV4MATH", "PV5MATH"),
         ~ weighted.mean(x = .x, w = W_FSTUWT, na.rm = T))) |>
  group_by(grade_category) |> 
  summarise(mean = sum(PV1MATH, PV2MATH, PV3MATH, PV4MATH, PV5MATH)/5)

# Compare aggregate mean to ACER (524 w/ sd of 95)
# Both give the right values.
pisa.mean.pv(
  pvlabel = paste0("PV", 1:5, "MATH"),
  data = PISA2003_uppercase)

Rrepest(data = PISA2003, 
        svy = "PISA", 
        est = est(c("mean", "std"), "pv@math"), 
        cm.weights = c(paste0("w_fstr", 1:80)))
...
PISA2000_uppercase <- PISA2000 

names(PISA2000_uppercase) <- toupper(names(PISA2000))

readmeans_2000_new <- pisa.mean.pv(pvlabel = paste0("PV", 1:5, "READ"), 
                                   by = "ST01Q01",
                                   data = PISA2000_uppercase)
...
readmeans_2003_new <- pisa.mean.pv(pvlabel = paste0("PV", 1:5, "READ"), 
                                   by = "ST01Q01",
                                   data = PISA2003)
#2006
...
readmeans_2006_new <- pisa.mean.pv(pvlabel = paste0("PV", 1:5, "READ"), 
                                   by = "ST01Q01",
                                   data = PISA2006)
#2009
...
readmeans_2009_new <- pisa.mean.pv(pvlabel = paste0("PV", 1:5, "READ"), 
                                   by = "ST01Q01",
                                   data = PISA2009_2)
...
readmeans_2012_new <- pisa.mean.pv(pvlabel = paste0("PV", 1:5, "READ"), 
                                   by = "ST01Q01",
                                   data = PISA2012)
...
readmeans_2015_new <- pisa.mean.pv(pvlabel = paste0("PV", 1:10, "READ"), 
                                   by = "ST001D01T",
                                   data = PISA2015)
...
readmeans_2018_new <- pisa.mean.pv(pvlabel = paste0("PV", 1:10, "READ"), 
                                   by = "ST001D01T",
                                   data = PISA2018)
#2022
...
readmeans_2022_new <- pisa.mean.pv(pvlabel = paste0("PV", 1:10, "READ"), 
                                   by = "ST001D01T",
                                   data = PISA2022)
...
# Add Year column
readmeans_2000_new$Year <- 2000
readmeans_2003_new$Year <- 2003
readmeans_2006_new$Year <- 2006
readmeans_2009_new$Year <- 2009
readmeans_2012_new$Year <- 2012
readmeans_2015_new$Year <- 2015
readmeans_2018_new$Year <- 2018
readmeans_2022_new$Year <- 2022
...
all_readmeans_new |> 
  ...
  geom_line() +
  scale_x_continuous(breaks = unique(all_means_new$Year)) + 
  labs(title = "Year 10's reading scores have been declining over the years",
       subtitle = glue("Average Australian Year 10 PISA score in Reading"),
       x = NULL) +
  theme_grattan() +
  grattan_y_continuous() +
  theme(plot.subtitle = element_markdown(lineheight = 1.1)) +
  geom_curve(data = . %>%  last(), 
             aes(x = 2019, 
                   y = 515, 
                   xend = 2022, 
                   yend = last(Mean) + 2),
...
nt_read_means_2012 <- Rrepest(data = PISA2012_nt, 
        ...
        cm.weights = c(paste0("w_fstr", 1:80)))
#2015
nt_read_means_2015 <- Rrepe
st(data = PISA_2015_nt, 
        svy = "PISA",
        est = est(c("mean", "std"), "pv@read"),
        cm.weights = c(paste0("w_fsturwt",1:80)))
#2018
