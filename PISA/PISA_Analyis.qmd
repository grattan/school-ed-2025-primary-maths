...
#2018
means_2015 <- Rrepest(data = PISA2018_Y10, 
        svy = "PISA",
        est = est(c("mean", "std"), "pv@math"),
        cm.weights = c(paste0("w_fsturwt", 1:80)))
#2022
...
yr10scores_ci <- ggplot(all_means, aes(x = Year, y = `b.mean.pv@math`)) +
  geom_point() +
  geom_line() +  
  geom_errorbar(aes(ymin = CI_Lower, ymax = CI_Upper), width = 0.2) +
...
yr10scores_ci +
  ...
  annotate("text", x = 2018.5, y = 500, label = "1 year delay due to COVID-19", size = 5, color = "darkgrey", hjust = 0.5)
